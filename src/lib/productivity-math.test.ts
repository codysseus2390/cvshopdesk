...
import { formatProductivity, overallProductivity, SHOP_PRODUCTIVITY_TECHNICIAN, type ProductivityInput } from "./productivity-math";
...
describe("overall mechanic productivity", () => {
  ...
  it("prefers an accepted overall period value", () => {
    const value = overallProductivity([
      ...
      row({ technician: SHOP_PRODUCTIVITY_TECHNICIAN, productivity_pct: 92.4, period_scope: "weekly" }),
    ], week, "weekly", "2026-09-16");
  ...
  it("returns unavailable for missing inputs or zero total worked hours", () => {
    ...
    expect(overallProductivity([row({ hours_billed: 8, hours_worked: 0 })], week, "weekly", "2026-09-16")).toBeNull();
  });
