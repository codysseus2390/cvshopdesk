...
describe("daily totals", () => {
  ...
  it("tracks coverage per metric and never fills a blank field with zero", () => {
    const totals = sumDaily([daily("2026-03-01", 1000, null, 5), daily("2026-03-02", 500, 3, 5)], 2);
  ...
  it("marks gp per car unavailable when gross profit and car count cover different days", () => {
    const totals = sumDaily([daily("2026-03-01", 1000, 2, null), daily("2026-03-02", 500, 2, 5)], 2);
    expect(totals.gross_profit).toBe(1500);
  ...
  it("excludes records dated after the shop's business day", () => {
    const totals = sumDaily([daily("2026-03-01", 1000, 2, 8), daily("2026-03-09", 9999, 9, 9)], 1, "2026-03-01");
...
describe("month to date", () => {
  ...
  it("ignores other months and future dates", () => {
    const result = monthToDate([daily("2026-02-28", 9999, 9, 9), daily("2026-03-20", 500, 1, 1)], "2026-03", "2026-03-03");
...
describe("year to date", () => {
  it("respects the latest accepted ytd snapshot instead of the daily sum", () => {
    const rows: MetricRow[] = [
      ...
      { business_date: "2026-02-28", scope: "ytd", gross_profit: 90_000, tires_sold: 400, car_count: 600 },
      { business_date: "2026-03-31", scope: "ytd", gross_profit: 120_000, tires_sold: 520, car_count: 800 },
    ];
  ...
  it("counts elapsed days of the year when summing daily rows", () => {
    const result = yearToDate([daily("2026-01-01", 1000, 2, 4), daily("2026-01-02", 1000, 2, 4)], "2026", "2026-03-01");
    expect(result.basis).toBe("daily-sum");
    expect(result.gross_profit).toBe(2000);
  ...
  it("never adds a cumulative snapshot to daily totals", () => {
    const rows: MetricRow[] = [
      daily("2026-01-05", 1000, 2, 4),
      { business_date: "2026-01-31", scope: "ytd", gross_profit: 40_000, tires_sold: 100, car_count: 200 },
    ];
