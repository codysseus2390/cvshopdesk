...
export const Route = createFileRoute("/_authenticated/account")({
  head: () => ({
    meta: [
      ...
      { property: "og:description", content: "Update your sign-in email, password and display name." },
...
function AccountPage() {
  ...
  async function verifyPassword(password: string) {
    if (!currentEmail) throw new Error("Your account email could not be read. Sign in again and retry.");
  ...
  async function saveEmail(e: React.FormEvent) {
    ...
    try {
      ...
      if (target === (currentEmail ?? "").toLowerCase()) throw new Error("That is already your email address.");
      ...
      setEmailNote({ ok: false, text: err instanceof Error ? err.message : "The email change did not go through." });
    } finally {
      setEmailBusy(false);
    }
  }
  ...
  async function savePassword(e: React.FormEvent) {
    ...
    try {
      ...
      if (error) throw new Error(error.message);
      setPasswordNote({ ok: true, text: "Your password is changed. Use it next time you sign in." });
      setCurrentPassword("");
      setNewPassword("");
      setConfirmPassword("");
    } catch (err) {
      setPasswordNote({ ok: false, text: err instanceof Error ? err.message : "The password change did not go through." });
    } finally {
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="space-y-1 text-sm">
              ...
              {role && <Badge variant="secondary">{role === "owner" ? "Owner" : role === "manager" ? "Manager" : "Staff"}</Badge>}
            </div>
            ...
            {nameNote && (
              <p className={`text-sm ${nameNote.ok ? "text-muted-foreground" : "text-destructive"}`}>{nameNote.text}</p>
            )}
          </CardContent>
        </Card>
          ...
          <CardContent>
            <form className="space-y-4" onSubmit={savePassword}>
              ...
              <div className="space-y-2">
                ...
                <Input id="new-pw" type="password" autoComplete="new-password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} required />
              </div>
              <div className="space-y-2">
                <Label htmlFor="new-pw2">Repeat new password</Label>
                <Input id="new-pw2" type="password" autoComplete="new-password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} required />
              </div>
              <Button type="submit" disabled={passwordBusy}>
                {passwordBusy ? "Saving…" : "Change password"}
              </Button>
              {passwordNote && (
                <p className={`text-sm ${passwordNote.ok ? "text-muted-foreground" : "text-destructive"}`}>{passwordNote.text}</p>
              )}
            </form>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <form className="grid gap-4 md:grid-cols-2" onSubmit={saveEmail}>
              ...
              <div className="space-y-2">
                ...
                <Input id="email-pw" type="password" autoComplete="current-password" value={emailPassword} onChange={(e) => setEmailPassword(e.target.value)} required />
              </div>
              <div className="md:col-span-2 space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  Your old email keeps working until you confirm the new one from the link sent to it.
                </p>
                {emailNote && (
                  <p className={`text-sm ${emailNote.ok ? "text-muted-foreground" : "text-destructive"}`}>{emailNote.text}</p>
                )}
              </div>
            </form>
          </CardContent>
