...
export function MetricCard({
  ...
  return (
    <Card className={`relative overflow-hidden border-t-2 hover:shadow-elevated ${greenAccent ? "border-t-secondary" : "border-t-primary"}`}>
      <CardContent className={size === "tv" ? "p-8" : "p-4 sm:p-5"}>
        <div className="flex items-center gap-3">
          <span className={`flex shrink-0 items-center justify-center rounded-full ${size === "tv" ? "h-14 w-14" : "h-10 w-10"} ${greenAccent ? "bg-secondary text-secondary-foreground" : "bg-primary text-primary-foreground"}`}>
        ...
        <p
          className={`font-display font-bold ${
            size === "tv" ? "mt-4 text-6xl" : "mt-3 text-4xl leading-none"
