...
export function HankChat({ compact = false }: { compact?: boolean }) {
  ...
  async function addFiles(files: File[]) {
    if (readingFilesRef.current || mutation.isPending) return;
  ...
  function submit(text?: string) {
    ...
    if ((!message && attachments.length === 0) || mutation.isPending || readingFilesRef.current)
      return;
    ...
    spokenRef.current = lastId ?? "awaiting-reply";
  ...
  return (
    <div
      className={
        compact ? "relative flex min-h-0 flex-1 flex-col" : "relative mx-auto w-full max-w-3xl"
      }
    >
      <div
        inert={voiceMode}
        className={
          compact ? "flex min-h-0 flex-1 flex-col" : "flex h-[calc(100dvh-180px)] min-h-96 flex-col"
        }
      >
        {!compact && (
        ...
        <Card
          className={
            compact
              ? "relative flex min-h-0 flex-1 flex-col overflow-hidden rounded-none border-0 shadow-none"
              : "noise-overlay relative flex min-h-0 flex-1 flex-col overflow-hidden"
          }
        >
          <CardContent
            ref={messagesRef}
            role="log"
            aria-label="Hank conversation"
            aria-live="polite"
            className="min-h-0 flex-1 space-y-5 overflow-y-auto overscroll-contain break-words p-4"
          >
            {isLoading && (
            ...
            {!isLoading && !historyError && messages.length === 0 && (
              <div className="py-6 text-center">
                ...
                <div
                  className={
                    compact
                      ? "mt-4 grid gap-2 text-left"
                      : "mt-4 grid gap-2 text-left sm:grid-cols-3"
                  }
                >
        ...
        <form
          ...
          className={`${compact ? "shrink-0 rounded-none border-x-0 border-b-0" : "mt-4 rounded-2xl shadow-card"} border bg-card p-3 transition-colors ${
            dragging ? "border-primary bg-primary/5" : "border-border/80"
          }`}
        >
          <AttachmentStrip
            items={attachments}
            disabled={mutation.isPending || readingFiles}
          ...
          <div className="flex items-end gap-2">
            ...
            <Textarea
              ...
              className="min-h-10 min-w-0 flex-1 resize-none border-transparent bg-muted/60 focus-visible:border-primary/40"
            />
            <Button
              type="button"
              variant="outline"
          ...
          <p className="mt-2 text-[11px] text-muted-foreground">
            {readingFiles
              ...
              : "Attach photos or PDFs · Enter to send · Shift + Enter for a new line"}
          </p>
