...
Status: API transport and disabled-by-default status webhook inbox implemented.
...
The built local route returns 503 when disabled. Migration 0025 was applied to
...
5. Finish the staging receiver configuration and verify real delivery before
   applying the additive production migration and enabling the live subscription.
