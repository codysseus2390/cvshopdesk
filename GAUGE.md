...
- Flag Lovable landmines immediately: any PR that removes or modifies `@lovable.dev/*` files or `.lovable/project.json` gets escalated to Cody before anything else happens.
...
## Done when
The PR comment says pass or fail per category with numbered findings that Cody can act on immediately.
